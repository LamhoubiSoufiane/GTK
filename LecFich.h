...
Mastructure *Traitement_Toggle_Button(Mastructure* box, xmlNodePtr child)
{
    Mastructure* Tgbutton, * ptr = NULL;
    MonUnion Info;
    if (!box)
    {
        printf("\nErreur, box n existe pas[Traitement_ButtonSimple]!!");
    ...
    Info.Tgbutton = Init_Toggle_Button(atoi((char*)xmlGetProp(child, "id")),
                                       (char*)xmlGetProp(child, "Nom"),
                                       (char*)xmlGetProp(child, "Icone"),
                                       atoi((char*)xmlGetProp(child, "Mnemonic")),
                                       atoi((char*)xmlGetProp(child, "PositionX")),
                                       ...
                                       atoi((char*)xmlGetProp(child, "Hauteur")),
                                       atoi((char*)xmlGetProp(child, "Largeur")));
    Tgbutton = Creer_Init_Cellule_Mastructure(Info, 8);
    if (box->Fils)
    {
        ptr = box->Fils;
        while (ptr->Frere) ptr = ptr->Frere;
    ...
    else box->Fils = Tgbutton;
    return((Mastructure*)box);
}

Mastructure *Traitement_checkButton(Mastructure *box,xmlNodePtr child)
{
    Mastructure *chk, * ptr = NULL;
    MonUnion Info;
    xmlNodePtr fils;
    if (!box)
    {
        printf("\nErreur, box n existe pas[Traitement_CheckButton]!!");
        exit(0);
    }
    Info.ChkBtn= Init_Check_Button(atoi((char*)xmlGetProp(child, "id")),
                                   (char*)xmlGetProp(child, "Nom"),
                                   (char*)xmlGetProp(child, "Icone"),
                                   atoi((char*)xmlGetProp(child, "Mnemonic")),
                                   atoi((char*)xmlGetProp(child, "Activate")),
                                   atoi((char*)xmlGetProp(child, "PositionX")),
                                   atoi((char*)xmlGetProp(child, "PositionY")),
                                   atoi((char*)xmlGetProp(child, "Hauteur")),
                                   atoi((char*)xmlGetProp(child, "Largeur")));
    for (fils = child->children; fils != NULL; fils = fils->next)
    {
        ...
        {
            if (!strcmp(fils->name, "Elem"))
            {
                Ajouter_Btn_Check(Info.ChkBtn,atoi((char*)xmlGetProp(fils, "id")),
                                  (char*)xmlGetProp(fils, "Nom"),
                                  (char*)xmlGetProp(fils, "Icone"),
                                  atoi((char*)xmlGetProp(fils, "Mnemonic")),
                                  atoi((char*)xmlGetProp(fils, "Activate")));
            }
            else {
                printf("\nErreur, format fichier invalide [traitement radbutton]");
                exit(0);
            }
        }
    }
    chk= Creer_Init_Cellule_Mastructure(Info,7);
    if(!box->Fils) box->Fils=chk;
    else{
        ptr=box->Fils;
        while(ptr->Frere) ptr=ptr->Frere;
        ptr->Frere=chk;
...
Mastructure *Traitement_RadButton(Mastructure *box, xmlNodePtr child)
{
    Mastructure *Rad, * ptr = NULL;
    MonUnion Info;
    xmlNodePtr fils;
    if (!box)
    {
        printf("\nErreur, box n existe pas[Traitement_RadButton]!!");
        exit(0);
    }
    Info.RadBtn= Init_Radio_Button(atoi((char*)xmlGetProp(child, "id")),
                        (char*)xmlGetProp(child, "Nom"),
                        atoi((char*)xmlGetProp(child, "PositionX")),
                        atoi((char*)xmlGetProp(child, "PositionY")),
                        atoi((char*)xmlGetProp(child, "Hauteur")),
    ...
    for (fils = child->children; fils != NULL; fils = fils->next)
    {
        if (fils->type == XML_ELEMENT_NODE)
        {
            if (!strcmp(fils->name, "Elem"))
            {
                Ajouter_Radio_Button(Info.RadBtn,(char*)xmlGetProp(fils, "Nom"));
            }
            else {
                printf("\nErreur, format fichier invalide [traitement radbutton]");
                exit(0);
    ...
    Rad= Creer_Init_Cellule_Mastructure(Info,6);
    if(!box->Fils) box->Fils=Rad;
    else{
        ptr=box->Fils;
        while(ptr->Frere) ptr=ptr->Frere;
        ptr->Frere=Rad;
...
Mastructure *Traitement_Frame(Mastructure *box, xmlNodePtr child)
{
    ...
    if (!box)
    {
        printf("\nErreur, box n existe pas[Traitement_Frame]!!");
        exit(0);
    }
    ...
    for (fils = child->children; fils != NULL; fils = fils->next)
    {
        if (fils->type == XML_ELEMENT_NODE)
        {
            if(!strcmp(fils->name,"Box")) F=Traitement_Box(F,fils);
            else if(!strcmp(fils->name,"Grid")) F=Traitement_Grid(F,fils);
            else
            {
...
Mastructure *Traitement_Box(Mastructure* fen, xmlNodePtr fils)
{
    ...
    for (child = fils->children; child != NULL; child = child->next)
    {
        if (child->type == XML_ELEMENT_NODE)
        {
            ...
            else if(!strcmp(child->name,"Frame")) nvbox = Traitement_Frame(nvbox, child);
            else if(!strcmp(child->name,"RadioButton")) nvbox = Traitement_RadButton(nvbox, child);
            else if(!strcmp(child->name,"CheckButton")) nvbox = Traitement_checkButton(nvbox, child);
            else if(!strcmp(child->name,"ToggleButton")) nvbox = Traitement_Toggle_Button(nvbox, child);
        }
...
void Traitement_Creation_Structure(Mastructure *str)
{
    ...
    switch (str->TypeInfo)
    {
        ...
        case 3 :
            break;
        case 4 :
            Creation_Frame(str->Information.fr,str->Information.fr->label);
            gtk_container_add(str->Information.fr->Fr,str->Fils->Information.Monbox->monbox);
            str->Information.fr->p=str->Fils->Information.Monbox->p;
            break;
        ...
        case 7:
            break;
        case 8:
            Creer_Toggle_Button(str->Information.Tgbutton);
            break;
