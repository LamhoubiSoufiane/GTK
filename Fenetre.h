...
*/
Fenetre *Init_Personnalise_Fenetre(int H, int L, int resizable, int posx, int posy, 
...
void Creer_Fenetre(Fenetre *fn)
{
    ...
    //gdk_pixbuf_new_from_file(fn->Icone, &error);
    /*if (error)
    {
        printf("\nErreur icone fenetre !! %s", error->message);
        exit(0);
    }*/
    //gtk_window_set_icon(GTK_WINDOW(fn->fenetre),icone);
    //Modifier la couleur de la fenetre
    //gdk_rgba_parse(&C,fn->Couleur);
    //gtk_widget_override_background_color(fn->fenetre, GTK_STATE_NORMAL,&C);
    //Ajouter l accel group à la fenetre
